...
def extract(spark: SparkSession, config: dict) -> dict:
    ...
    return {
        "employee":   read_hive(spark, db, "dim_employee"),
...
def enrich(dfs: dict):
    ...
    df_person_slim = dfs["person"].select(
    ...
    df = dfs["employee"].alias("e") \
        ...
        .join(df_person_slim.alias("p"),
              F.col("e.businessentityid") == F.col("p.person_id"), "left") \
        ...
        .select(
            F.col("e.businessentityid").alias("employee_id"),
            F.concat_ws(" ", F.col("p.firstname"), F.col("p.lastname")).alias("full_name"),
            F.col("e.jobtitle").alias("job_title"),
            F.col("d.dept_name").alias("department_name"),
            F.col("e.hiredate").alias("hire_date"),
            F.col("pay.latest_pay_rate"),
            F.col("h.dept_history_count"),
        )

    logger.info("Joined HR rows: %d", df.count())
    return df
    ...
    df_result = df_enriched \
        ...
        .withColumn("pay_bracket",
            F.when(F.col("latest_pay_rate") < 15,  "Low (<15)")
             ...
             .otherwise("Top (40+)")) \
        .withColumn("pay_rank_in_dept",
            F.rank().over(
                Window.partitionBy("department_name").orderBy(F.desc("latest_pay_rate")))) \
        .withColumn("tenure_rank_in_dept",
            F.rank().over(
                Window.partitionBy("department_name").orderBy(F.desc("tenure_years")))) \
        .drop("dept_history_count") \
        .withColumn("load_timestamp", F.current_timestamp())

    # Verify department filter held
    depts = [r[0] for r in df_result.select("department_name").distinct().collect()]
